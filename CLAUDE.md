...
**Produção** (desde 11/09/2026): https://meajudaai-jet.vercel.app, com deploy automático a cada push na `main` de `leonardochalhoub/meajudaai` (remote `leonardo`; o `origin` é o repositório antigo, `DRoqueProgrammer/meajudaai`). O Site URL do Supabase Auth aponta para lá e `NEXT_PUBLIC_SITE_URL` também. Nunca volte nenhum dos dois para `localhost`: os e-mails de confirmação e de recuperação de senha saem com esse host. Detalhes no README, seção "Produção (Vercel)".
