...
**Testes agora (11/09, madrugada):** `npm test` (unidade, sem banco — 365 verdes); `RUN_INTEGRATION=1 npx vitest run --config vitest.integration.config.ts tests/fatia1 tests/fatia2 tests/anuncios tests/agenda tests/faturamento tests/admin tests/vitrine` (banco real — 219 verdes em 32 arquivos; `npm run test:integration` inteiro ainda tem o vermelho antigo do banner do SysAdmin, ADR 0009); `bash scripts/regressao/rodar-fatia1.sh` (build de produção + 10 passos no Chrome, **10/10**, o cancelamento sem o aviso do D-031); `node scripts/regressao/qr-pix.mjs` (lê o QR Pix com jsQR, 4/4). **Não rode o build com o `next dev` no ar no mesmo checkout** — os dois escrevem em `.next`.
