...
export interface EventoPopoverEvento {
  ...
  servico: { descricao: string; preco_tipo: string; preco_valor: number; status: string; clienteNome?: string | null } | null;
}

/**
 * Card efêmero com o resumo de um evento da agenda. Renderiza como overlay
 ...
 * ver. Fecha ao clicar fora (no fundo) ou em "Ver serviço". Só o essencial
 * pra decidir se vale abrir o serviço; qualquer ação (aceitar, cancelar,
 * log) fica na página em `href`, não aqui.
 */
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-ink/40 p-4" onClick={onFechar}>
      ...
      >
        ...
        {servico ? (
          ...
          <p className="mt-2 text-sm text-muted">Horário livre — ainda sem cliente.</p>
        )}
