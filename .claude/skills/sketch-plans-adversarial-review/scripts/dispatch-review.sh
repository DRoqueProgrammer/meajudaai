...
#
# Usage:
#   dispatch-review.sh --adversary codex|kimi|gemini|claude [--dir sketch] [--out F]
#   dispatch-review.sh --help
# Engine command is env-overridable for testing: CVG_CODEX_CMD / CVG_KIMI_CMD /
#   CVG_GEMINI_CMD / CVG_CLAUDE_CMD (default: the bare binary). CVG_TIMEOUT_SECS
#   (default 300). The gemini engine authenticates itself via GEMINI_API_KEY in
#   the environment — export it before running, the referee never reads secrets.
#
...
case "$ADVERSARY" in
  ...
  gemini) FAMILY=google;    CMD="${CVG_GEMINI_CMD:-gemini}"; KIND=gemini ;;
  claude) FAMILY=anthropic; CMD="${CVG_CLAUDE_CMD:-claude}"; KIND=claude ;;
  *) uerr "unknown adversary '$ADVERSARY' (codex|kimi|gemini|claude)" ;;
esac

[ -d "$SKETCH_DIR" ] || uerr "no swimlane tree at $SKETCH_DIR/"
if [ "$FAMILY" = "$AUTHOR_FAMILY" ]; then
  echo "WARN: adversary '$ADVERSARY' is the SAME family as the author ($AUTHOR_FAMILY) — weak (self-preference). Prefer codex, kimi or gemini." >&2
fi
...
case "$KIND" in
  ...
  # --approval-mode plan é o modo read-only do Gemini CLI; --skip-trust é
