...
- [ ] **Lista de cidades via IBGE** (§7) — hoje é a lista fixa `lib/cidades.ts` (14 cidades hardcoded). Pattern pronto pra portar: `refs/foco-contabil/lib/ibge.ts` (fetch na API pública do IBGE + cache em localStorage + `BR_STATES` + `UF_CENTROIDS`) e `refs/foco-contabil/components/city-picker.tsx` (combobox com busca + filtro por UF), ambos originados do `caixa-forte-app`. Leonardo também citou um repo "amazing school" com o mesmo padrão — não está clonado em `refs/` ainda, mas não bloqueia: o do foco-contabil já resolve sozinho. Aplica no cadastro (cliente/prestador) e no perfil.
- [ ] **`/mapa` está 100% da v1** — a página inteira ainda é sobre `vagas`/diária/`workspace` (local_aprox, vaga_local); não existe nada aí pro modelo P2V v2. Pra todo prestador/cliente v2 ela só mostra "Nenhuma vaga com localização no mapa ainda". Precisa virar: cliente vê prestadores próximos no mapa (via `buscar_prestadores_proximos`), prestador vê algo equivalente (área de atuação/clientes, a definir).
- [ ] Hero card ainda considerado "feio" por Leonardo — pendência de design, não de função (cabeçalho, citação, relógio e previsão já funcionam).
- [ ] SysAdmin e Administrador caem no `/inicio` genérico (Hero + CTA de publicar vaga) — Leonardo confirmou ao vivo que isso não faz sentido pra nenhum dos dois; precisam de dashboard próprio (KPIs + atalhos), sem Hero.
