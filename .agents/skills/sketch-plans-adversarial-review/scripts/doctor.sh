...
ENGINES="codex:openai:CVG_CODEX_CMD kimi:moonshot:CVG_KIMI_CMD gemini:google:CVG_GEMINI_CMD claude:anthropic:CVG_CLAUDE_CMD"
...
if [ "$PASS" -ge 2 ] && [ "$XFAM" -ge 1 ]; then
  echo "READY — Pass 4 can dispatch a cross-family adversary (cvg review --adversary codex|kimi|gemini)."
  echo "DOCTOR=OK"; exit 0
else
  echo "NOT READY — need >= 2 engines and >= 1 cross-family (codex, kimi or gemini). Install one." >&2
